    ...
    <form action="/dashboard/apotek/{{ $apotek->idApotek }}" method="POST">
        ...
        @method('PUT')
            ...
            <div class="row mb-3">
                ...
                <input type="text" class="form-control @error('linkgmaps') is-invalid @enderror" name="linkgmaps" style="font-size: 12px;" required value="{{ old('linkgmaps', $apotek->linkgmaps) }}"> 
                @error('linkgmaps')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
